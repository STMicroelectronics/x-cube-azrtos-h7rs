 ...
 *
 * definition of the value to detect no adress management case
 */
...
SFDP_StatusTypeDef JEDEC_Basic_ManageEnableSequence(EXTMEM_DRIVER_NOR_SFDP_ObjectTypeDef *Object);
...
/**
 * @brief this function read and check the SFDP header and ajust 
...
SFDP_StatusTypeDef SFDP_GetHeader(EXTMEM_DRIVER_NOR_SFDP_ObjectTypeDef *Object, SFPD_HeaderTypeDef *sfdp_header)
{
           ...
           ; index++) 
  {
    ...
    for (uint8_t IExt = 0;
         (IExt < 2) && (retr == EXTMEM_SFDP_ERROR_SIGNATURE); IExt++)
    {
      ...
      {
        /* Config 1 is invalide so exit the loop */
        break;
      }
    }
  }
...
#error "the assembly instruction is not available"
  ...
  */
  ...
  if(0u != (Object->sfpd_private.Sfdp_table_mask & (uint32_t)SFPD_PARAMID_STATUS_CONTROL_CONFIG_REGISTER_MAP))
  {
    ...
    if (0u != JEDEC_SCCR_Map.Param_DWORD.D5.WIPBitAvailable)
    {
      ...
      {
        /* in that case there is no adress to manage, the value EXTMEM_ADDRESS_NONE is used to detect the difference */
  ...
  */
      ...
      && (EXTMEM_LINK_CONFIG_8LINES == Object->sfpd_private.Config))
  {
    ...
    else
    {
      ...
      if (EXTMEM_SFDP_OK == sfpd_enter_octal_mode(Object))
      {
        ...
        if ((uint32_t)SFPD_PARAMID_STATUS_CONTROL_CONFIG_REGISTER_MAP == (Object->sfpd_private.Sfdp_table_mask & (uint32_t)SFPD_PARAMID_STATUS_CONTROL_CONFIG_REGISTER_MAP))
        {
          dummyCycles = JEDEC_SCCR_Map.Param_DWORD.D3.NbDummyCyclesReadRegisterCmdVolatileRegister_8D8D8D_mode;
        }
        ...
        (JEDEC_Basic.size > 16u))
    {
      ...
      SAL_XSPI_SET_COMMANDEXTENSION(Object->sfpd_private.SALObject, JEDEC_Basic.Params.Param_DWORD.D18.OctalDTRCommandExtension);
    }
  }
      ...
      ((PHY_LINK_8D8D8D == Object->sfpd_private.DriverInfo.SpiPhyLink) || (PHY_LINK_8S8D8D == Object->sfpd_private.DriverInfo.SpiPhyLink)))
  {
    ...
    if (JEDEC_XSPI10.Param_DWORD.D6._8D8D8DDefaultPOR_DummyCycle != 0)
    {
    ...
    if ((ClockOut >= CLOCK_200MHZ) && (JEDEC_XSPI10.Param_DWORD.D4.Operation200Mhz_DummyCycle != 0))
    {
    ...
    else if ((ClockOut >= CLOCK_166MHZ) && (JEDEC_XSPI10.Param_DWORD.D5.Operation166Mhz_DummyCycle != 0))
    {
    ...
    else if ((ClockOut >= CLOCK_133MHZ) && (JEDEC_XSPI10.Param_DWORD.D5.Operation133Mhz_DummyCycle != 0))
    {
...
SFDP_StatusTypeDef JEDEC_Basic_ManageQuadEnableRequierement(EXTMEM_DRIVER_NOR_SFDP_ObjectTypeDef *Object)
{
  ...
  case 0x2u: {/* 010b: QE is bit 6 of status register 1. It is set via Write Status with one data byte where bit 6 is one. 
    ...
    if (HAL_OK != SAL_XSPI_SendReadCommand(&Object->sfpd_private.SALObject, 0x5u, (uint8_t *)&localValue, 1u))
    {
      retr = EXTMEM_SFDP_ERROR_DRIVER;
      goto error;
    }
    ...
    localValue = localValue | 0x40;
    
    /* 4 - write the status register with QPI mode to 1 */
    if (HAL_OK != SAL_XSPI_CommandSendData(&Object->sfpd_private.SALObject, 0x1u, (uint8_t *)&localValue, 1u))
    {
      retr = EXTMEM_SFDP_ERROR_DRIVER;
      goto error;
    }
  ...
  case 0x3u: // 011b: QE is bit 7 of status register 2. It is set via Write status register 2 instruction 3Eh with one data byte where bit 7 is one. It is cleared via Write status register 2 instruction 3Eh with one data byte where bit 7 is zero. The status register 2 is read using instruction 3Fh.
    break;
  case 0x4u: // 100b: QE is bit 1 of status register 2. It is set via Write Status with two data bytes where bit 1 of the second byte is one. It is cleared via Write Status with two data bytes where bit 1 of the second byte is zero. In contrast to the 001b code, writing one byte to the status register does not modify status register 2.
    break;
...
SFDP_StatusTypeDef JEDEC_Basic_ManageEnableSequence(EXTMEM_DRIVER_NOR_SFDP_ObjectTypeDef *Object)
{
  ...
  uint8_t instruction = 0x00;
    ...
    if ((JEDEC_Basic.Params.Param_DWORD.D15._4S4S4S_EnableSequence & 0x1) == 0x1)
    {
      instruction = 0x38;
    }
    /* x_x1xxb: issue instruction 35h */
    else if ((JEDEC_Basic.Params.Param_DWORD.D15._4S4S4S_EnableSequence & 0x3) == 0x3)
    {
      instruction = 0x35;
    }
...
*/
    if (HAL_OK != SAL_XSPI_SendReadCommand(&Object->sfpd_private.SALObject, instruction, NULL, 0u))
    {
      retr = EXTMEM_SFDP_ERROR_DRIVER;
      goto error;
    }
