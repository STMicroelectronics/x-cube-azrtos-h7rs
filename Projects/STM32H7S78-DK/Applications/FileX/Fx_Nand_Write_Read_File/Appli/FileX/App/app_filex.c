...
/* Total sectors */
#define FX_NAND_DISK_TOTAL_SECTORS                ((((CUSTOM_TOTAL_BLOCKS) * CUSTOM_PHYSICAL_PAGES_PER_BLOCK * CUSTOM_BYTES_PER_PHYSICAL_PAGE)/ FX_NAND_DISK_SECTOR_SIZE)-1)

/* Number of FATs */
#define FX_NAND_DISK_NUMBER_OF_FATS                2
...
/* Sectors per cluster */
#define FX_NAND_DISK_SECTORS_PER_CLUSTER           8
...
ALIGN_32BYTES(uint32_t media_memory[2048]);
...
ULONG fx_lx_nand_custom_driver_buffer[(7 * CUSTOM_TOTAL_BLOCKS + 4 + 4 * (CUSTOM_BYTES_PER_PHYSICAL_PAGE + CUSTOM_SPARE_BYTES_PER_PAGE)) / sizeof(ULONG)];
...
void fx_app_thread_entry(ULONG thread_input)
{
  ...
  printf("FileX/LevelX Custom NAND W25N01GVXX Application Start.\n");
  ...
  /* Open the w25n01gvxx NAND Flash disk driver.  */
