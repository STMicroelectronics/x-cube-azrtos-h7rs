...
board(doesn't run on STM32H7S78-DK board unless a custom board is added). It demonstrates how to create a Fat File system
on the NOR flash using FileX alongside LevelX. The application is designed to execute file operations on the Winbond W25Q128JV
NOR flash device, the code provides all required software code for properly managing it.

 - This application has been tested using a MIKROE Flash 6 Click board embedding a Winbond W25Q128JV NOR flash device configured as below:

    - MemoryDataWidth = 1 bit
    - PageSize = 256 bytes
    - BlockSize = 64 KB
    - BlockNbr = 256
    - Sector Size: = 4 KB
  ...
  - User can design his own hardware according to the following SPI4 GPIO configuration:
...
At this stage, all FileX resources are created, the NOR flash is initialized and a single thread is created:
...
RTOS, ThreadX, FileX, LevelX, File System, NOR, W25Q128JV, FAT32

### <b>Hardware and Software environment</b>

  - This application runs on STM32H7S7L8xx devices.
  - This application has been tested using STMod+ including MIKROE Flash 6 Click board embedding a Winbond W25Q128JV NOR flash device and plugged on
    STM32H7S78-DK boards Revision: MB1736-H7S7L8-D01 and can be easily tailored to any other supported device and development board.
