...
uint8_t USB_State = STOP_USB_DEVICE;
uint8_t SD_State = STOP_SD_CARD;
...
static VOID app_ux_device_thread_entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    switch (USB_Device_State_Msg)
    {
      case START_USB_DEVICE:
        USB_State = START_USB_DEVICE;
        break;
      case STOP_USB_DEVICE:
        USB_State = STOP_USB_DEVICE;
        break;
      case START_SD_CARD:
        SD_State = START_SD_CARD;
        break;
      case STOP_SD_CARD:
        SD_State = STOP_SD_CARD;
        break;
      default:
        /*Error*/
        Error_Handler();
    ...
    if ((USB_State == START_USB_DEVICE) && (SD_State == START_SD_CARD) &&
    ...
    else if ((USB_State == STOP_USB_DEVICE) || ((SD_State == STOP_SD_CARD) &&
             (HAL_GPIO_ReadPin(SD_DETECT_GPIO_Port, SD_DETECT_Pin) == GPIO_PIN_SET) &&
             (USB_Device_State == START_USB_DEVICE)))
    {
      USB_Device_State = STOP_USB_DEVICE;
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
  ...
  {
    USB_Device_State_Msg = (HAL_GPIO_ReadPin(SD_DETECT_GPIO_Port, SD_DETECT_Pin) == GPIO_PIN_RESET) ? START_SD_CARD : STOP_SD_CARD;
